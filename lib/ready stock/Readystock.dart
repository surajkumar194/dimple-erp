...
class _ReadyStockScreenState extends State<ReadyStockScreen> {
  ...
  Future<void> _loadDataFromFirebase() async {
    ...
    try {
      final snapshot =
          await _firestore.collection('ready_stock_items').orderBy('sr').get();
  ...
  Future<double> _calculateAverageStock(String itemId) async {
    try {
      ...
      final itemDoc =
          await _firestore.collection('ready_stock_items').doc(itemId).get();
              ...
              child: Column(
                children: [
                  Text("Average Stock (30 Days)",
                      style:
                          TextStyle(fontSize: 12, color: Colors.purple[700])),
                  ...
                  Text("units",
                      style:
                          TextStyle(fontSize: 12, color: Colors.purple[600])),
                ],
              ),
            ),
          ],
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(isEdit ? "Edit Ready Stock Item" : "Add New Item",
                          style: const TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Colors.white,
                              fontSize: 20)),
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildModernField(codeCtrl, "DPL Code",
                            Icons.qr_code, required: true),
                        const SizedBox(height: 16),
                        _buildModernField(detailCtrl, "Detail",
                            Icons.description, required: true),
                        const SizedBox(height: 16),
                        _buildModernField(pillerCtrl, "Piller No.",
                            Icons.pin, required: true),
                        ...
                        Row(children: [
                          Expanded(
                              child: _buildModernField(inCtrl, "IN",
                                  Icons.add_box, isNumber: true)),
                          const SizedBox(width: 12),
                          Expanded(
                              child: _buildModernField(incomingCtrl,
                                  "Incoming", Icons.input, isNumber: true)),
                        ]),
                        const SizedBox(height: 16),
                        Row(children: [
                          Expanded(
                              child: _buildModernField(outCtrl, "OUT",
                                  Icons.remove_circle, isNumber: true)),
                          const SizedBox(width: 12),
                          Expanded(
                              child: _buildModernField(balCtrl, "Balance",
                                  Icons.account_balance, isNumber: true)),
                        ]),
                        const SizedBox(height: 16),
                        _buildModernField(
  ...
  Future<void> _uploadExcelFile() async {
    ...
    try {
      ...
      bool hasRequired =
          ['code', 'detail', 'piller_no'].every((h) => headerRow.contains(h));
      if (!hasRequired) {
        _showSnackBar(
      ...
      for (var item in importedItems) {
        ...
        if (existingQuery.docs.isNotEmpty) {
          ...
          var docRef = _firestore.collection('ready_stock_items').doc();
          ...
          child: Container(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    Text("Error",
                        style: TextStyle(
                            fontSize: 8, color: Colors.red[600]))
                  ]))));
      ...
      body: _isLoading
          ...
          : Column(
              children: [
                ...
                // EXACT HEADER AS PER YOUR IMAGE
                Container(
                  padding:
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            return Container(
                              ...
                              child: Row(children: [
                                ...
                                _cell(item["bal"].toString(), 90,
                                    color: Colors.green[900],
                                    bold: true,
                                    fontSize: 16.sp),
                                _cell(item["dateEdit"]?.toString() ?? "", 90),
                                _cell("–", 130), // MOVING ITEMS
