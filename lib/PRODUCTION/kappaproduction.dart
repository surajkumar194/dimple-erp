...
const List<_Stage> _kStages = [
  ...
  _Stage(
    'kappa_cutting',
    'Kappa Cutting / Assembly (if applicable)',
    Icons.content_cut,
    Color(0xFF169a8d),
  ),
...
class _KappaProductionScreenState extends State<KappaProductionScreen>
  ...
  Future<void> _finalSave(String type, int productIndex) async {
    ...
    try {
      ...
      final shouldLock =
          _allDone() && updated.every((p) => p['finalStatus'] != null);
      ...
      if (mounted) {
        final productName =
            (_kappaProducts[productIndex]['productName'] ?? 'Product')
                .toString();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(
  ...
  Widget? _buildBottomBar() {
    ...
    final currentIndex = (_tabs?.index ?? 0).clamp(
      0,
      _kappaProducts.isEmpty ? 0 : _kappaProducts.length - 1,
    );

    final currentProductDone = _kappaProducts.isEmpty
        ? false
        : _allDoneForProduct(currentIndex);

    final alreadySent = _kappaProducts.isEmpty
        ? false
        : (_kappaProducts[currentIndex]['finalStatus'] != null);

    final canAct =
        currentProductDone && !_saving && !alreadySent && !_isFullyLocked;
    ...
    if (alreadySent) {
      final status = _kappaProducts[currentIndex]['finalStatus'] ?? '';
  ...
  void _showConfirmDialog(String type, int productIndex) {
    ...
    final productName =
        (_kappaProducts[productIndex]['productName'] ?? 'Product').toString();
    showDialog(
      context: context,
  ...
  Widget _buildEmpty() => Center(
    child: Column(
      ...
      children: [
        ...
        Text(
          'This order has no Kappa Box category products.',
  ...
  Widget _tabBar() => Container(
    ...
    child: TabBar(
      ...
      tabs: List.generate(_kappaProducts.length, (i) {
        final name = (_kappaProducts[i]['productName'] ?? 'Product ${i + 1}')
            .toString();
        final done = _kStages
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              // Auto switch to next incomplete product tab
              if (_allDoneForProduct(i) &&
                  _tabs != null &&
                  _kappaProducts.length > 1) {
                for (int next = i + 1; next < _kappaProducts.length; next++) {
                  if (!_allDoneForProduct(next)) {
                    final productName =
                        (_kappaProducts[next]['productName'] ??
                                'Product ${next + 1}')
                            .toString();
