...
// ═══════════════════════════════════════════════════════════════
//  MDF PRODUCTION LIST SCREEN
//  Dashboard se yahan aao → order select karo → MdfProductionScreen
// ═══════════════════════════════════════════════════════════════
...
class _MdfProductionListScreenState extends State<MdfProductionListScreen> {
  ...
  // ─── Count MDF products in order ─────────────────────────────
  int _mdfCount(List products) =>
      products.where((p) => (p['productCategory'] ?? '') == 'MDF').length;

  // ─── Count completed stages across all MDF products ──────────
  ...
  int _totalStages(List products) {
    const stageCount = 6;
    return _mdfCount(products) * stageCount;
  }
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (ctx, snap) {
                ...
                // filter: only orders that have at least 1 MDF product
                ...
                final filtered = allDocs.where((doc) {
                  ...
                  final products = data['products'] is List
                      ...
                      : [];
                  final hasMdf = _mdfCount(products) > 0;
                  if (!hasMdf) return false;
  ...
  builder: (context, snap) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => MdfProductionScreen(orderId: orderId),
        ),
      ),
      child: Container(
