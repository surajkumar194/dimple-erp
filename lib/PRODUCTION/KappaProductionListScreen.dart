...
import 'package:dimple_erp/PRODUCTION/Kappaproductionreport.dart';
...
class _KappaProductionListScreenState extends State<KappaProductionListScreen> {
  String _search = '';
  ...
  bool _isKappa(dynamic p) =>
      (p['productCategory'] ?? '').toString().toLowerCase().contains('kappa');

  int _kappaCount(List products) => products.where(_isKappa).length;
  ...
  int _totalStages(List products) => _kappaCount(products) * _stages.length;

  int _minProductDoneStages(List products) {
    int min = _stages.length;
    bool hasKappa = false;
    for (final p in products) {
      if (!_isKappa(p)) continue;
      hasKappa = true;
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => const KappaProductionReportScreen(),
              ),
            ),
          ),
        ],
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (ctx, snap) {
          ...
          final kappaDocs = allDocs.where((doc) {
            final data = doc.data() as Map<String, dynamic>;
            final products = data['products'] is List
                ? data['products'] as List
                : [];
            return _kappaCount(products) > 0;
          }).toList();
          ...
          return Column(
            children: [
              ...
              Expanded(
                child: kappaDocs.isEmpty
                    ? _buildEmpty()
                    : Builder(
                        builder: (context) {
                          final filtered = kappaDocs.where((doc) {
                            final data = doc.data() as Map<String, dynamic>;
  ...
  Widget _orderCard(String orderId, Map<String, dynamic> data) {
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        final kappaProducts = prod.where(_isKappa).toList();
        final productProgressChips = kappaProducts.map((p) {
          final kappaProd = _kappaProduction(p);
          final done = _stages
              .where((s) => kappaProd[s]?['done'] == true)
              .length;
                ...
                if (productProgressChips.isNotEmpty)
                  Padding(
                    ...
                    child: Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: productProgressChips
                          .map(_buildProductChip)
                          .toList(),
                    ),
                  ),
        ...
        children: [
          ...
          if (chip.finalStatus != null)
            Container(
              ...
              decoration: BoxDecoration(
                color: chip.finalStatus == 'DISPATCH'
                    ? Colors.green
                    : Colors.blue,
