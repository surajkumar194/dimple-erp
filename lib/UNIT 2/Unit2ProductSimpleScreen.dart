...
class _Unit2ProductSimpleScreenState extends State<Unit2ProductSimpleScreen> {
  ...
  List<_GroupedProduct> _groupProducts(
    ...
    // _groupProducts method mein, jahan entry add karta hai, 
// ek condition add karo dispatched check ki:
...
for (final p in products) {
  ...
  // ─── YEH NEW LINE ADD KARO ──────────────────────────────────
  final statusKey = _processStatusKey(name);
  final isDispatched = data['${statusKey}_dispatched'] == true;
  if (isDispatched) continue; // dispatched entries skip
  // ────────────────────────────────────────────────────────────
                  ...
                  return Column(
                    children: [
                      _buildDetailRow(
                        ...
                        index: idx,
                        isEven: idx % 2 == 0,
                        isLast: idx == g.entries.length - 1 && !isEntryExpanded,
                        isExpanded: isEntryExpanded,
                        onTap: () {
                          setState(() {
                            _expandedEntryIndex = isEntryExpanded ? null : idx;
                          });
                        },
                      ),
                      AnimatedSize(
            ...
            SizedBox(
              ...
              child: Center(
                child: entry.imageUrl != null
                    ? GestureDetector(
                        onTap: () => _openImages(context, entry.allImages),
                        child: Container(
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(10),
                            border: Border.all(
...
class _ProcessChecklistState extends State<_ProcessChecklist> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        final processStatus = Map<String, dynamic>.from(data[_statusKey] ?? {});
        ...
        return Column(
          ...
          children: [
            ...
            _ProcessRow(
              ...
              enabled: pvcEnabled,
              remarkController: TextEditingController(
                text: (processStatus['pvc/butter']?['remark'] ?? '') as String,
              ),
  ...
  Future<void> _saveProcess(
    ...
    await FirebaseFirestore.instance
        ...
        .set({
          _statusKey: {
            // ← uses processStatus_<productName>
            key: {
              'done': val,
              'qty': qty,
              'remark': remark,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          const Text(
            'Products will appear once job cards are created.',
